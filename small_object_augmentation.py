...
'''
paper : https://www.mdpi.com/1424-8220/22/19/7663
code : https://github.com/ZhenglinZhou/Data_Augmentation_Zoo_for_Object_Detection/blob/master/augmentation_zoo/SmallObjectAugmentation.py
'''

class SmallObjectAugmentation(object):
    def __init__(self, thresh=64*64, prob=0.5, copy_times=3, epochs=30, all_objects=False, one_object=False, seed=None):
        """
    ...
    def issmallobject(self, h, w):
        '''
        객체의 크기(h*w) 가 임계값(self.thresh) 보다 작은지 여부 
        '''
        if h * w <= self.thresh:
            return True
        else:
    ...
    def compute_overlap(self, annot_a, annot_b):
        '''
        두 bbox 간의 겹침 여부를 계산
        (annot 형식은 (x_min, y_min, x_max, y_max) 형태)
        '''
    ...
    def donot_overlap(self, new_annot, annots):
        '''
        새로운 annotation이 기존 annotation과 겹침 여부를 확인
        '''
        for annot in annots:
    ...
    def create_copy_annot(self, h, w, annot, annots):
        '''
        annotation을 새로운 랜덤 위치에 겹치지 않게 복사
        annot : 복사할 원본 annotation
        annots : 기존 annotation 리스트
        '''
